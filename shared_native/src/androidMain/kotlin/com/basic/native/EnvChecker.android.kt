...
import buildkonfig.BuildConfig_com_basic_nativeLibs
...
//apk完整性基础验证上一次校验时间
var preVerifyTime = 0L

//apk完整性基础验证上一次校验结果
var preVerifyPass = false

//apk完整性基础验证并发锁
val apkEnvVerifyLock = Any()
actual inline fun checkEnv() {
    synchronized(apkEnvVerifyLock) {
        ...
        try {
            ...
            if (stringBuffer.toString() != BuildConfig_com_basic_nativeLibs.APK_VERIFY_CODE) {
                throw RuntimeException("unknow error")
            }
