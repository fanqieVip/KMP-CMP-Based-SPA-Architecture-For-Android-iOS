...
2.  **内容画像**：识别补丁核心内容。
3.  **映射目标**：
    - **UI 展现/基础组件/实战** -> `UI组件使用指南及规范.md`
    - **协作流程/提问模板/页面结构** -> `UI开发工作流规范.md`
    - **图片资源/切图/Import 规范** -> `资源与切图规范.md`
    - **架构设计/模块职责/依赖关系** -> `架构设计文档.md`
    - **API 接口/路由/网络/SPI** -> `架构api文档.md`
    - **通用代码风格/文件头/Git 规范** -> `通用代码规范.md`
    - **进化机制/补丁规约/自维护** -> `AI-Skill自动升级指南.md`
    - **环境同步/IDE/构建/CocoaPods** -> `IDE环境同步指南.md` 或 `Pod依赖使用指南与规范.md`
...
- ❌ **权限锁死**：AI 严禁擅自合并自生成的补丁。必须由用户下达指令。
