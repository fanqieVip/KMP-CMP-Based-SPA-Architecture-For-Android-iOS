...
class ChinaCommonDistributionServiceImpl : CommonDistributionService {
    ...
    private suspend fun getOaid(): String = suspendCancellableCoroutine { continuation ->
        DeviceID.getOAID(Utils.getApp(), object : IGetter {
            override fun onOAIDGetComplete(result: String?) {
                continuation.resumeIfActive(result.takeUnless { it.isNullOrEmpty() } ?: CACHE_OAID_FAIL_TAG)
            }
    ...
    private companion object {
        /** OAID 缓存键。 */
        const val CACHE_OAID_KEY = "com.basic.base/CACHE_OAID_KEY"

        /** Android ID 缓存键。 */
        const val CACHE_ANDROID_ID_KEY = "com.basic.base/CACHE_ANDROID_KEY"

        /** OAID 获取失败的缓存标记。 */
        const val CACHE_OAID_FAIL_TAG = "FAIL"
    }
}
