...
// AES-256-CBC protected source runtime utility.
internal object ProtectSrcAes256cbc {

    private const val BLOCK_SIZE = 16
    private const val KEY_SIZE = 32

    /**
     * Decrypts raw AES-CBC ciphertext bytes and returns raw plain bytes.
     */
    fun decryptBytes(encryptedBytes: ByteArray, key: ByteArray, iv: ByteArray): ByteArray {
        val ivBytes = validateAndConvertIV(iv)

        require(encryptedBytes.size % BLOCK_SIZE == 0) { "" }
     ...
     * Validates a raw IV.
     */
    private fun validateAndConvertIV(iv: ByteArray): ByteArray {
        require(iv.size == 16) { "" }
        return iv.copyOf()
    }
    ...
    private fun unpadPKCS7(data: ByteArray): ByteArray {
        ...
        require(padLength in 1..BLOCK_SIZE) { "" }

        for (i in data.size - padLength until data.size) {
            require(data[i].toInt() and 0xFF == padLength) { "" }
        }
